        ...
        <nav className={`fixed top-0 left-0 right-0 z-50 bg-white shadow-lg transition-all duration-300`}>
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex items-center justify-between h-20">
                    ...
                    >
                        <div className="w-10 h-10 bg-[#4A1D7A] rounded-xl flex items-center justify-center">
                    ...
                    <div className="hidden lg:flex items-center space-x-8">
                        {navItems.map((item, index) => (
                            <motion.button
                                ...
                                className="text-gray-700 dark:text-gray-200 hover:text-[#4A1D7A] font-medium transition-colors duration-300 relative group"
                                initial={{ opacity: 0, y: -20 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ duration: 0.6, delay: index * 0.1 }}
                            >
                                {item.name}
                                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#4A1D7A] transition-all duration-300 group-hover:w-full"></span>
                            </motion.button>
                        ))}

                        {/* CTA Button */}
                        <motion.button
                            className="px-6 py-3 bg-[#4A1D7A] text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105"
                        ...
                        <div className="px-6 py-4 space-y-4">
                            {navItems.map((item, index) => (
                                <motion.button
                                    ...
                                    className="block text-gray-700 dark:text-gray-200 hover:text-[#4A1D7A] font-medium py-2 transition-colors duration-300 w-full text-left"
                                    initial={{ opacity: 0, x: -20 }}
                                    animate={{ opacity: 1, x: 0 }}
                            ...
                            {/* Mobile CTA Button */}
                            <motion.button
                                className="w-full px-6 py-3 bg-[#4A1D7A] text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300"
                                initial={{ opacity: 0, y: 20 }}
                                animate={{ opacity: 1, y: 0 }}
