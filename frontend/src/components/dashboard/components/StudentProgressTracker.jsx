...
const StudentProgressTracker = ({ currentStage }) => {
    ...
    return (
        <motion.div 
            ...
            className="w-full bg-white dark:bg-[#231A2E] p-6 rounded-lg shadow-md mb-8 overflow-hidden"
        >
                        ...
                        <div key={stage.id} className="relative z-10 flex flex-col items-center group flex-1">
                            ...
                            <div 
                                className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border-[3px] transition-all duration-500 shadow-sm
                                    ${isCompleted 
                                        ...
                                        : isCurrent 
                                            ? 'bg-white dark:bg-[#231A2E] border-[#4A1D7A] text-[#4A1D7A] dark:text-[#9B6FCC] scale-125 shadow-[0_0_15px_rgba(168,85,247,0.4)]' 
                                            : 'bg-white dark:bg-[#231A2E] border-gray-300 dark:border-gray-600 text-gray-400 dark:text-gray-300'
                                    }`}
                            >
                                {isCompleted ? (
                            ...
                            <span 
                                className={`absolute top-14 text-xs md:text-sm font-semibold uppercase tracking-wider text-center w-32 transition-all duration-300
                                    ${isCompleted ? 'text-[#4A1D7A] dark:text-[#9B6FCC]' 
