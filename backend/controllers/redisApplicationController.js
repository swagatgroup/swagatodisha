...
class RedisApplicationController {
    ...
    async createApplication(req, res) {
        ...
        try {
            console.log('Creating application with Redis workflow for user:', req.user._id);
            ...
            res.status(500).json({
                success: false,
                message: 'Failed to start application submission',
    ...
    async getApplicationStatus(req, res) {
        try {
            ...
            if (workflowStatus.submission && workflowStatus.submission.status === 'completed') {
            ...
            const completedSteps = Object.values(workflowStatus)
    ...
    async uploadDocument(req, res) {
        try {
            ...
            const jobId = await queueProcessor.addToQueue('document_processing', documentData, 1);
