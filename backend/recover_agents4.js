...
async function recoverAgents() {
  try {
    ...
    // First, fix the one we already created partially (nagnarendra92@gmail.com)
    await agentsCol.deleteOne({ 'personalDetails.email': "nagnarendra92@gmail.com" });
    ...
    for (const email of emailsToRecover) {
      ...
      if (!existing) {
        ...
        userId = existing._id;
        // update referralCode on user if missing
        if (!existing.referralCode) {
            await usersCol.updateOne({ _id: userId }, { $set: { referralCode } });
        }
      }
