...
const connectDB = async () => {
    try {
        ...
        if (error.message.includes('ECONNREFUSED')) {
            console.log('💡 Solution: Make sure MongoDB is running locally or check your connection string');
            console.log('   - Start MongoDB: mongod');
            console.log('   - Or update MONGODB_URI in your .env file');
        }

        if (error.message.includes('authentication failed')) {
            console.log('💡 Solution: Check your MongoDB credentials in the connection string');
        }
