...
class WorkflowEngine {
    ...
    async startApplicationSubmission(userId, applicationData) {
        ...
        try {
            ...
            if (!lockValue) {
                throw new Error('Another submission is already in progress for this user');
            }
            ...
            const result = await this.executeWorkflow('application_submission', submissionId, {
                userId,
                applicationData,
                submissionId
            });
    ...
    async initializeWorkflowState(submissionId, userId, applicationData) {
        ...
        await redisManager.set(`workflow:${submissionId}`, JSON.stringify(workflowState));
    ...
    async executeWorkflow(workflowName, submissionId, context) {
        ...
        for (const step of workflow.steps) {
            try {
                ...
                await this.updateWorkflowStep(submissionId, step, 'completed', stepResult);
                ...
                console.error(`Step ${step} failed for submission ${submissionId}:`, error);

                // Mark step as failed
                await this.updateWorkflowStep(submissionId, step, 'failed', { error: error.message });

                // Handle retry logic
                const retryResult = await this.handleStepRetry(workflowName, submissionId, step, error, context);
                if (!retryResult.success) {
                    throw new Error(`Step ${step} failed after retries: ${error.message}`);
                }
    ...
    async validateApplicationData(context) {
        ...
        if (applicationData.personalDetails?.aadharNumber) {
            const existingApplication = await StudentApplication.findOne({
                'personalDetails.aadharNumber': applicationData.personalDetails.aadharNumber
            });
    ...
    async processDocuments(context) {
        ...
        for (const docType of documentTypes) {
            if (documents[docType]) {
                try {
                    const documentResult = await this.processDocument(docType, documents[docType], submissionId);
    ...
    async processDocument(docType, documentData, submissionId) {
        ...
        const jobId = await redisManager.addToQueue('document_processing', {
            docType,
            documentData,
            submissionId,
        ...
        const result = await this.waitForJobCompletion('document_processing', jobId, 30000);
    ...
    async createApplicationInDatabase(context) {
        ...
        if (applicationData.referralCode) {
            const referrer = await User.findOne({ referralCode: applicationData.referralCode });
            if (referrer) {
                referralInfo = {
    ...
    async sendNotifications(context) {
        ...
        for (const notification of notifications) {
            await redisManager.publishEvent(notification.channel, notification.event, notification.data);
        }
    ...
    async completeApplicationSubmission(context) {
        ...
        await redisManager.publishEvent('workflow:completed', 'application_submitted', {
            submissionId,
            applicationId: application.applicationId,
    ...
    async handleStepRetry(workflowName, submissionId, step, error, context) {
        ...
        console.log(`Retrying step ${step} in ${delay}ms (attempt ${parseInt(retryCount) + 1})`);
    ...
    async handleWorkflowError(submissionId, error) {
        // Update workflow state
        await this.updateWorkflowStep(submissionId, 'error', 'failed', { error: error.message });
    ...
    async resumeWorkflow(submissionId) {
        ...
        return await this.executeWorkflow('application_submission', submissionId, context);
    }
}
