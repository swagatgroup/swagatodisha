...
const testApplicationEndpoints = async () => {
    ...
    if (submittedResult.success && submittedResult.status === 200) {
        ...
        logTest('Get Submitted Applications', 'PASS', `${appCount} applications found`);
    } else {
...
const testContentEndpoints = async () => {
    ...
    let galleryResult = { success: false, status: 401 };
    if (authToken) {
        galleryResult = await makeRequest('GET', '/gallery', null, { Authorization: `Bearer ${authToken}` });
    }
    ...
    if (galleryResult.success && galleryResult.status === 200) {
        ...
        logTest('Gallery Access', 'FAIL', `Status: ${galleryResult.status}, Error: ${JSON.stringify(galleryResult.error)}`);
    }
    ...
    if (coursesResult.success && coursesResult.status === 200) {
        ...
        logTest('Courses Endpoint', 'FAIL', `Status: ${coursesResult.status}, Error: ${JSON.stringify(coursesResult.error)}`);
    }
...
module.exports ={ runAllTests, testBasicEndpoints, testAuthentication, testApplicationEndpoints, makeRequest };
