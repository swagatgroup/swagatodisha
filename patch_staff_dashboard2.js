...
const regex = /<motion\.div\s+initial={{ opacity: 0, y: 20 }}\s+animate={{ opacity: 1, y: 0 }}\s+transition={{ delay: 0\.15 }}\s+className="mb-8 w-full lg:w-1\/2 mx-auto"\s+>\s+\{\/\* Assigned Agents Section \*\/\}[\s\S]*?\}\)\}\s+<\/div>\s+<\/motion\.div>\s+\)\}\s+\{\/\* ── Student View Toggle ── \*\/\}/g;
...
content = content.replace(regex, assignedAgentsUI);
